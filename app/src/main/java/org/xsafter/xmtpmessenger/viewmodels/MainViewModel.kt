    ...
    fun initializeClient() {
        runBlocking {
            withContext(Dispatchers.IO) {
                ...
                if (keys == null) {
                    try {
                        sharedViewModel.client = Client().create(account, ClientManager.CLIENT_OPTIONS)

                        val serializedKeys = PrivateKeyBundleV1Builder.encodeData(sharedViewModel.client.privateKeyBundleV1)
                    ...
                    sharedViewModel.client = Client().buildFrom(bundle = keys!!, options = options)
                }
            }
        }
    }
